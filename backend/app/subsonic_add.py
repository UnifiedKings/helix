...
def _parse_duration_text_to_ms(value: Any) -> int:
    if value is None:
        return 0
...
def _extract_duration_ms(track: Dict[str, Any]) -> int:
    ...
    for key in ("duration", "length"):
        parsed = _parse_duration_text_to_ms(track.get(key))
        if parsed > 0:
            return parsed
...
async def add_album(
    ...
    try:
        if client is not None and album_title and album_artist:
            ...
            for track in tracks:
                ...
                yt_duration_ms = _extract_duration_ms(track)
                if yt_duration_ms > 0:
    ...
    for t in tracks:
        ...
        duration_ms = _extract_duration_ms(t)
