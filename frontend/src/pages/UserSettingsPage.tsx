...
] as const satisfies ReadonlyArray<readonly [keyof UserSettings, string, string]>

const DEFAULT_PAYLOAD: UserSettingsPayload = {
  settings: {
    ...DEFAULT_PALETTE,
...
function ColorField({
  ...
  return (
    <label className="settings-palette-field">
      <span className="settings-palette-copy">
        <strong>{label}</strong>
        <span>{description}</span>
      </span>
      <span className="settings-palette-control">
...
export function UserSettingsPage() {
  ...
  const [payload, setPayload] = useState<UserSettingsPayload>(DEFAULT_PAYLOAD)
  const [draft, setDraft] = useState<UserSettingsPayload['settings']>(DEFAULT_PAYLOAD.settings)
  ...
  useEffect(() => {
    ...
    // Appearance controls preview immediately. Custom CSS remains opt-in so a
    // half-written CSS rule cannot unexpectedly break the settings page while
    // the user is typing.
    const liveSettings = {
      ...draft,
      advanced_custom_css: previewing
        ? draft.advanced_custom_css
        : payload.settings.advanced_custom_css,
    }

    window.dispatchEvent(new CustomEvent('helix-user-settings-updated', {
      detail: { settings: liveSettings, limits: payload.limits } satisfies UserSettingsPayload,
    }))
  }, [draft, payload.limits, payload.settings.advanced_custom_css, previewing, loading])

  function update<K extends keyof UserSettingsPayload['settings']>(key: K, value: UserSettingsPayload['settings'][K]) {
  ...
  async function save() {
    ...
    setStatus('')
    try {
      const patch: Partial<UserSettingsPayload['settings']> = {}
      for (const key of Object.keys(draft) as Array<keyof typeof draft>) {
        if (draft[key] !== payload.settings[key]) (patch as Record<string, unknown>)[String(key)] = draft[key]
      }
  ...
  async function resetAll() {
    ...
    setError('')
    try {
      const next = await api.resetUserSettings()
      setPayload(next)
      setDraft(next.settings)
      setPreviewing(false)
      window.dispatchEvent(new CustomEvent('helix-user-settings-updated', { detail: next }))
    ...
    <div className="settings-page settings-page-user">
      <header className="settings-page-header">
        <div>
          ...
          <h1>Settings</h1>
          <p>Customize how Helix looks and behaves for your account.</p>
        </div>
      ...
      <div className="settings-layout">
        <nav className="settings-section-nav" aria-label="Settings sections">
          {SECTIONS.map(([key, label]) => (
            <button type="button" className={section === key ? 'active' : ''} key={key} onClick={() => setSection(key)}>{label}</button>
          ))}
        </nav>
        ...
        <section className="settings-section-content" aria-busy={loading}>
          {section === 'appearance' ? <>
              ...
              <div className="settings-control-row">
                ...
                <div className="settings-accent-picker">
                  <div className="settings-color-presets">
                    {ACCENT_PRESETS.map(([label, color]) => <button type="button" key={color} className={draft.appearance_accent_color.toLowerCase() === color ? 'active' : ''} style={{ '--swatch-color': color } as CSSProperties} aria-label={label} title={label} onClick={() => update('appearance_accent_color', color)}><span /></button>)}
                  </div>
            ...
            <div className="settings-card">
              <div className="settings-control-row">
                <div>
                  <strong>Helix logo color</strong>
                  <span>Let the sidebar logo follow the primary color, or give it its own color.</span>
                </div>
                <div className="settings-logo-color-controls">
                  <label className="settings-inline-toggle">
                    <input
                      type="checkbox"
                      checked={draft.appearance_logo_follow_accent}
                      onChange={(event) => update('appearance_logo_follow_accent', event.target.checked)}
                    />
                    <span>Follow primary</span>
                  </label>
                  <div className="settings-color-control">
                    <input
                      aria-label="Custom Helix logo color"
                      type="color"
                      value={draft.appearance_logo_color}
                      disabled={draft.appearance_logo_follow_accent}
                      onChange={(event) => update('appearance_logo_color', event.target.value)}
                    />
                    <code>{draft.appearance_logo_follow_accent ? draft.appearance_accent_color : draft.appearance_logo_color}</code>
                  </div>
                </div>
              </div>
            </div>
          ...
          {section === 'search' ? <>
            ...
            <div className="settings-card">
              <label className="settings-control-row"><div><strong>Default search source</strong><span>You can still change this at any time from the Search page.</span></div><select value={draft.search_default_mode} onChange={(event) => update('search_default_mode', event.target.value as 'hybrid' | 'subsonic' | 'ytmusic')}><option value="hybrid">All</option><option value="subsonic">Library</option><option value="ytmusic">YTMusic</option></select></label>
              <label className="settings-control-row"><div><strong>Default results section</strong><span>The tab shown first after a search.</span></div><select value={draft.search_default_tab} onChange={(event) => update('search_default_tab', event.target.value as 'songs' | 'albums' | 'artists')}><option value="songs">Songs</option><option value="albums">Albums</option><option value="artists">Artists</option></select></label>
            </div>
          </> : null}
          ...
          {section === 'queue' ? <>
            ...
            <div className="settings-card">
              <div className="settings-control-row settings-control-row-stack-mobile">
                <div>
                  <strong>Add to queue placement</strong>
                  <span>Choose whether Add to queue puts new music at the end or directly after the currently playing track.</span>
                </div>
                <Segmented
                  value={draft.queue_add_position}
                  options={[['append', 'End of queue'], ['next', 'Play next']]}
                  onChange={(value) => update('queue_add_position', value)}
                />
              </div>
          ...
          {section === 'notifications' ? <>
            ...
            <div className="settings-card">
              ...
              <label className="settings-control-row"><div><strong>Notification duration</strong><span>How long lightweight confirmations stay visible.</span></div><select value={draft.notifications_duration} onChange={(event) => update('notifications_duration', event.target.value as 'short' | 'normal' | 'long')}><option value="short">Short</option><option value="normal">Normal</option><option value="long">Long</option></select></label>
            </div>
          </> : null}
