...
function SimilarArtistSeedPicker({ config, onChange }: { config: StationConfig; onChange: (config: StationConfig) => void }) {
  ...
  return <div className="station-song-seed-picker"><div className="station-config-field"><span className="station-config-label">Seed artist <strong>Required</strong></span><small>Search YouTube Music and choose the exact artist this radio should be built around.</small>{seedArtist ? <div className="station-song-seed-selected"><strong><ArtistLink artist={seedArtist} /></strong></div> : null}<div className="station-song-seed-search"><input value={query} placeholder={seedArtist ? 'Search to change seed artist' : 'Search for an artist'} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void runSearch() } }} /><button type="button" className="btn-secondary" disabled={searching || !query.trim()} onClick={() => void runSearch()}>{searching ? 'Searching…' : 'Search'}</button></div>{searchError ? <small className="error-text">{searchError}</small> : null}{results.length ? <div className="station-song-seed-results">{results.map((artist) => { const browseId = String(artist.browse_id || artist.artist_id || ''); return <button type="button" className="station-song-seed-result" key={`${browseId}:${artist.name}`} onClick={() => chooseSeed(artist)}><strong><ArtistLink artist={artist.name} browseId={browseId} /></strong>{artist.subscriber_count ? <span>{artist.subscriber_count}</span> : null}</button> })}</div> : null}</div></div>
}
