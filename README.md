...
Helix supports trusted Python plugins that can add entirely new station types with their own recommendation logic, tuning controls, anti-repeat behavior, and optional cover-art strategy.
...
Mount your plugin directory into the configured path, then restart Helix after adding or changing a provider.

Custom station plugins run inside the Helix container with the same Python permissions as Helix itself, so only install plugins you wrote yourself or fully trust.

For the complete plugin API, required functions, registration methods, configuration schema, cover hooks, examples, and troubleshooting, see:

**[Custom Station Provider Guide](CUSTOM_STATIONS.md)**
